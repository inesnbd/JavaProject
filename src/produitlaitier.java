...
public class produitlaitier extends JFrame {
	...
	public produitlaitier() {
		...
		try {
			...
			String sql = "SELECT * FROM produitlaitiers";
			
			// Etape 4 éxécution requête
			rs=(ResultSet) st.executeQuery(sql);
			
			String[] titre = {"id","produitlaitiers","descriptions","prix ","quantites"}; // on creer nos colonnes du tableau
		      String data[][] = new String[10][5];
		    
		      int i = 0;
		      while (rs.next()) {
		        int id = rs.getInt("id");
		        String nom = rs.getString("produitlaitiers");
